...
class HolidayPassbookController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($data['passbookOwner']) {
            ActivityLogger::log('view_admin_holiday_passbook', 'Admin melihat buku tabungan Hari Raya user.', null, [
                'user_id' => $data['passbookOwner']['id'],
                'filters' => $data['filters'],
            ]);
        }

        return Inertia::render('Admin/Holiday/Passbook', $data);
    }
    ...
    public function pdf(Request $request)
    {
        ...
        ActivityLogger::log('download_admin_holiday_passbook_pdf', 'Admin download PDF buku tabungan Hari Raya user.', null, [
            'user_id' => $data['passbookOwner']['id'],
            'filters' => $data['filters'],
        ]);
        ...
        return Pdf::loadView('pdf.holiday-passbook', $data)
            ...
            ->download('buku-tabungan-hari-raya-user-' . $data['passbookOwner']['id'] . '-' . now()->format('Ymd-His') . '.pdf');
    }
    ...
    private function passbookData(Request $request): array
    {
        ...
        $programs = $owner
            ? HolidaySavingProgram::whereHas('participants', function ($query) use ($owner) {
        ...
        $totalDeductionApplied = $owner ? $this->totalDeductionApplied($programs, $selectedProgram, $owner->id) : 0;
        ...
        return [
            ...
            'summary' => [
                ...
                'deduction' => $owner ? $this->deductionSummary($selectedProgram, $owner->id) : null,
    ...
    private function deductionSummary(?HolidaySavingProgram $program, int $userId): ?array
    {
        ...
        $participant = \App\Models\HolidaySavingParticipant::where('holiday_saving_program_id', $program->id)
    ...
    {
        if ($selectedProgram) {
            $participant = \App\Models\HolidaySavingParticipant::where('holiday_saving_program_id', $selectedProgram->id)
                ->where('user_id', $userId)
                ->first();
