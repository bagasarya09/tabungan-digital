...
export default function Passbook({ users = [], passbookOwner = null, programs = [], rows = [], summary = {}, filters = {} }) {
    ...
    const applyFilter = (e) => {
        e.preventDefault();
        router.get('/admin/holiday/passbooks', filterData, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const resetFilter = () => {
        ...
        setFilterData(reset);
        router.get('/admin/holiday/passbooks', reset);
    };

    const pdfUrl = () => `/admin/holiday/passbooks/pdf?${new URLSearchParams(filterData).toString()}`;

    return (
        <AdminLayout>
            <Head title="Cetak Buku Tabungan Hari Raya" />

            <style>{`
                @media print {
                    body * { visibility: hidden; }
                    #admin-holiday-passbook-print-area, #admin-holiday-passbook-print-area * { visibility: visible; }
                    #admin-holiday-passbook-print-area { position: absolute; left: 0; top: 0; width: 100%; background: white; }
            ...
            <div className="mx-auto max-w-7xl space-y-6">
                <div className="no-print">
                    ...
                    <h1 className="mt-1 text-2xl font-semibold text-[#1A1A1A] sm:text-3xl">
                        Cetak Buku Tabungan Hari Raya
                    </h1>
                    <p className="mt-2 text-sm text-[#5D5B54]">
                        Pilih peserta untuk mencetak buku Tabungan Hari Raya beserta rincian potongannya.
                    </p>
                </div>
                    ...
                    <div className="grid gap-3 md:grid-cols-4 lg:grid-cols-6">
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => setFilterData({ ...filterData, user_id: e.target.value, program_id: '' })}
                ...
                {!passbookOwner ? (
                    <div className="rounded-xl border border-dashed border-[#E5E3DF] bg-white p-10 text-center shadow-[0_1px_2px_rgba(15,15,15,0.04)]">
                        <h2 className="text-lg font-semibold text-[#1A1A1A]">Pilih user terlebih dahulu.</h2>
                        <p className="mt-2 text-sm text-[#787671]">
                            Setelah user dipilih, admin bisa memilih program Hari Raya dan mencetak buku tabungan peserta.
                        </p>
                    </div>
                ) : (
                    <>
                        ...
                        <section id="admin-holiday-passbook-print-area" className="rounded-xl border border-[#E5E3DF] bg-white p-4 shadow-[0_1px_2px_rgba(15,15,15,0.04)] sm:p-6">
                            <div className="border-b border-[#E5E3DF] pb-5">
                                ...
                                <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-5">
                                    <div><p className="text-[#787671]">Nama User</p><p className="font-semibold text-[#1A1A1A]">{passbookOwner.name}</p></div>
                                    <div><p className="text-[#787671]">No Anggota</p><p className="font-semibold text-[#1A1A1A]">{passbookOwner.member_number || passbookOwner.email}</p></div>
                                    <div><p className="text-[#787671]">Program</p><p className="font-semibold text-[#1A1A1A]">{summary.programLabel}</p></div>
                                    <div><p className="text-[#787671]">Periode</p><p className="font-semibold text-[#1A1A1A]">{summary.periodLabel}</p></div>
                                    <div><p className="text-[#787671]">Tanggal Cetak</p><p className="font-semibold text-[#1A1A1A]">{summary.printedAt}</p></div>
                                </div>
                            </div>
